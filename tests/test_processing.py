...
def test_assert_filter_by_state(short_data_other, short_data_canceled): #аргументы это фикстуры
...
def test_assert_sort_by_date(example_data, answer, answer_rev_falls, same_data, same_data_answer, data_error): #аргументы это фикстуры
    assert sort_by_date(example_data) == answer
    assert sort_by_date(example=example_data, order=True) == answer
    assert sort_by_date(example=example_data, order=False) == answer_rev_falls
    assert sort_by_date(example=example_data, order=None) == 'Задайте порядок сортировки'
    assert sort_by_date(example=example_data, order='None') == 'Задайте порядок сортировки'
    assert sort_by_date(example=example_data, order=12345) == 'Задайте порядок сортировки'
    assert sort_by_date(example=same_data) == same_data_answer
    assert sort_by_date(data_error) == 'Неверный формат даты'
    assert sort_by_date(example=data_error, order=False) == 'Неверный формат даты'
    assert sort_by_date([]) == 'Используйте список по образцу [{"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"}]'
